...
namespace simplex
{
    ...
    class my_matrix
    {
      public:
        ...
        my_matrix &
        operator=(const my_matrix & p_matrix);

        my_matrix &
        operator=(my_matrix && p_matrix) noexcept;

        void
        set_data(unsigned int p_row_index
        ...
        void
        swap_line(unsigned int p_row_index_1
                 ,unsigned int p_row_index_2
                 );

        void
        swap_column(unsigned int p_column_index_1
        ...
        bool
        operator==(const my_matrix & p_matrix) const;
...
//-------------------------------------------------------------------------
    template <typename T>
    void
    my_matrix<T>::set_data(unsigned int p_row_index
                          ,unsigned int p_column_index
                          ,const T & p_value
                          )
    {
...
//-------------------------------------------------------------------------
    template <typename T>
    void
    my_matrix<T>::initialize(const T & a)
    {
...
//-------------------------------------------------------------------------
    ...
    {
        my_matrix l_extracted_matrix(m_height - 1,
                                     m_width - 1
                                    );
        unsigned int p_extracted_row_index = 0;
        unsigned int p_extracted_column_index = 0;

        for (unsigned int p_current_row_index = 0;
             p_current_row_index < m_height;
             p_current_row_index++
                )
        {
            for (unsigned int p_current_column_index = 0;
                 p_current_column_index < m_width;
                 p_current_column_index++
                    )
            {
                if (p_current_row_index != p_line_index && p_current_column_index != p_column_index)
                {
                    l_extracted_matrix.set_data(p_extracted_row_index,
                                                p_extracted_column_index,
                                                get_data(p_current_row_index,
                                                         p_current_column_index
                                                        )
                                               );
                }
...
//-------------------------------------------------------------------------
                                ...
                                ) const
    {
        ...
        std::tuple<T, unsigned int, unsigned int> l_result(get_data(p_min_height,
                                                                    p_min_width
                                                                   ),
                                                           p_min_height,
                                                           p_min_width
        ...
        for (unsigned int l_row_index = p_min_height;
             l_row_index < m_height;
             ++l_row_index
                )
        {
            for (unsigned l_column_index = p_min_width;
                 l_column_index < m_width;
                 ++l_column_index
                    )
            {
                if (std::get<0>(l_result) < get_data(l_row_index,
                                                     l_column_index
                                                    ))
                {
                    l_result = std::make_tuple(get_data(l_row_index,
                                                        l_column_index
                                                       ),
                                               l_row_index,
                                               l_column_index
...
//-------------------------------------------------------------------------
                                    ...
                                    ) const
    {
        ...
        std::tuple<T, unsigned int, unsigned int> l_result(std::abs(get_data(p_min_height,
                                                                             p_min_width
                                                                            )
                                                                   ),
                                                           p_min_height,
        ...
        {
            for (unsigned int l_column_index = p_min_width;
                 l_column_index < m_width;
                 l_column_index++
                    )
            {
                if (std::get<0>(l_result) < std::abs(get_data(l_row_index,
                                                              l_column_index
                                                             )
                                                    ))
                {
                    l_result = std::make_tuple(std::abs(get_data(l_row_index,
                                                                 l_column_index
                                                                )
                                                       ),
                                               l_row_index,
...
//-------------------------------------------------------------------------
                                ...
                                ) const
    {
        ...
        std::tuple<T, unsigned int> l_result(get_data(p_row_index,
                                                      p_column_index
                                                     ),
                                             p_row_index
                                            );

        for (unsigned int l_row_index = p_row_index;
             l_row_index < m_height;
             l_row_index++
                )
        {
...
//-------------------------------------------------------------------------
                                    ...
                                    ) const
    {
        ...
        std::tuple<T, unsigned int> l_result(std::abs(get_data(p_row_index,
                                                               p_column_index
                                                              )
                                                     ),
                                             p_row_index
...
//-------------------------------------------------------------------------
    template <typename T>
    void
    my_matrix<T>::swap_line(unsigned int p_row_index_1
                           ,unsigned int p_row_index_2
                           )
    {
        ...
        for (unsigned int l_column_index = 0;
             l_column_index < m_width;
             l_column_index++
                )
        {
            l_tmp_data = get_data(p_row_index_1,
                                  l_column_index
                                 );
            set_data(p_row_index_1,
                     l_column_index,
                     get_data(p_row_index_2,
                              l_column_index
                             )
                    );
            set_data(p_row_index_2,
                     l_column_index,
                     l_tmp_data
                    );
        }
...
//-------------------------------------------------------------------------
    template <typename T>
    void
    my_matrix<T>::swap_column(unsigned int p_column_index_1
                             ,unsigned int p_column_index_2
                             )
    {
        ...
        {
            l_tmp_data = get_data(l_row_index,
                                  p_column_index_1
                                 );
            set_data(l_row_index,
                     p_column_index_1,
                     get_data(l_row_index,
                              p_column_index_2
                             )
                    );
            set_data(l_row_index,
...
//-------------------------------------------------------------------------
    ...
    my_matrix<T>::mult(const my_matrix & p_matrix)
    {
        ...
        {
            throw quicky_exception::quicky_logic_exception("my_matrix.class: fatal error ! you try to multiplicate two matrix with incompatible sizes",
                                                           __LINE__,
                                                           __FILE__
                                                          );
        }
        my_matrix l_result(m_height,
                           p_matrix.get_width()
                          );
        T l_total;
        ...
        {
            for (unsigned int l_column_index = 0;
                 l_column_index < p_matrix.get_width();
                 l_column_index++
                    )
            {
                l_total = 0;
                for (unsigned int l_mixed_index = 0;
                     l_mixed_index < m_width;
                     l_mixed_index++
                        )
                {
                    l_total += get_data(l_row_index,
                                        l_mixed_index
                                       ) * p_matrix.get_data(l_mixed_index,
                                                             l_column_index
                                                            );
                }
                l_result.set_data(l_row_index,
                                  l_column_index,
                                  l_total
                                 );
            }
...
//-------------------------------------------------------------------------
    ...
    my_matrix<T>::to_string() const
    {
                ...
                )
        {
            ...
            {
                l_string += std::to_string(get_data(l_row_index,
                                                    l_column_index
                                                   )
                                          ) + "\t";
            }
...
//-------------------------------------------------------------------------
    ...
    my_matrix<T>::operator==(const my_matrix & p_matrix) const
    {
                ...
                )
        {
            ...
            {
                if (get_data(l_row_index,
                             l_column_index
                            ) != p_matrix.get_data(l_row_index,
                                                   l_column_index
                                                  ))
                {
                    return false;
